...
title: BMAD-METHOD：突破性敏捷 AI 驱动开发方法
date: 2026-01-08
categories: [AI-AGENT]
tags: [ai, ai-agent, sh]
published: true
...
全新改造的 BMAD V6 安装器现已包括对创建、安装和分享**自定义模块、代理、工作流、模板和工具**的内置支持！
...
* 自定义内容概览 - [https://docs.bmad-method.org](https://docs.bmad-method.org)
* 安装指南 - [https://docs.bmad-method.org](https://docs.bmad-method.org)
* 内容详细文档（代理、模块、工作流等） - [https://docs.bmad-method.org](https://docs.bmad-method.org)
* 示例模块下载与安装（示例） - [https://docs.bmad-method.org](https://docs.bmad-method.org) ([GitHub][1])

---

## **🚀 BMAD 方法是什么？**

**BMAD-METHOD（Breakthrough Method for Agile AI Driven Development）** 是一个**AI 驱动的敏捷开发框架**，专为现代软件开发设计，通过多个分工明确的 AI 代理协同完成开发生命周期中的不同任务。该框架通过预定义的工作流和验证过的敏捷流程，帮助开发团队从需求分析到最终实现，不断推进项目进度。([GitHub][1])

与传统**单一通用 AI 辅助编程工具**不同，BMAD 方法：
...
1. **分析阶段（Analysis）** — 可选：调研、探索、头脑风暴
2. **规划阶段（Planning）** — 产品需求文档（PRD）、技术规范
3. **解决方案阶段（Solutioning）** — 架构与 UX 设计
4. **实现阶段（Implementation）** — 敏捷开发、功能交付 ([GitHub][1])
...
完整贡献指南见仓库中的 `CONTRIBUTING.md`。:contentReference[oaicite:23]{index=23}
...
本项目采用 **MIT 许可证**：
https://github.com/bmad-code-org/BMAD-METHOD/blob/main/LICENSE :contentReference[oaicite:24]{index=24}

BMAD™ 和 BMAD-METHOD™ 是 BMad Code, LLC 的商标。:contentReference[oaicite:25]{index=25}

---

## **📌 项目定位总结**

**BMAD-METHOD** 是一个用于 **AI 协作全生命周期软件开发的框架与方法论**：通过结构化的 **多角色 AI 代理 + 工作流 + 文档模板**，把敏捷最佳实践与 AI 协同整合起来，让开发过程更智能、更规范、更可复现。:contentReference[oaicite:26]{index=26}
